...
class MQLtoMySQL
{
	...
	static async select( mql, db )
	{
		...
		var binds = [];
		var sql = 'SELECT ';
		if( mql['options']['count'] && mql.options.count == true )
		{
		...
		for( var tkey in mql['data'] )
		{
			...
			for( var ckey in table )
			{
				...
				switch( column.flag )
				{
					...
					case MQL.SET:
						sql += lend+cglue+lend+'('+column.value+') AS '+ckey;
						cglue = ', ';
						break;
		...
		if( mql.slice != null )
		{
			sql += ' ' + lend + 'LIMIT ' + mql.slice.limit;
			sql += ' OFFSET ' + mql.slice.offset;
		}
		...
		{
			return await db.select( sql, binds );
		}
	}
		...
		try
		{
			...
			while( queue.length > 0 && t < queue.length )
			{
				...
				if( cancreate && Object.keys( columns ).length > 0 )
				{
					...
					// print_r( array( 'table' => targetName, 'cols' => columns, 'where' => where ) );//exit;
					if( !mql['tables'][ targetName ]['insert'] && where.length > 0 )
					{
						/*
						...
						var [rows, fields] = await db.select( sql, binds );
						if( rows.length > 0 )
						{
							await db.update( mql['tables'][ targetName ]['table'], columns, where );
						}
					...
					{
						// print_r( columns );
						var result = await db.insert( mql['tables'][ targetName ]['table'], [columns] );
						id = result.insertId;
		...
		try
		{
			...
			for( var tkey in mql.data )
			{
				...
				if( candelete )
				{
					...
					result = db.delete( mql.tables[tkey].table, [where] );
				}
				else
				{
					if( debug )
