...
String.isString = function (v) { return v.constructor === String };
...
module.exports = class MQLtoMySQL
{
	...
	static async select(mql, db)
	{
		...
		else
		{
			...
			{
				var temp = await this.select(target.table);
				var tsql;
	...
	static findColumnValue(column, data, linkage = { v: false })
	{
		...
		case MQL.JOIN:
			...
			{
				linkage.v = true;
				let path = column.value.split('.');

				let table = data[path[0]];
				let ncol = null;
				...
				// console.log( "ncol: ", path );
				//echo "res: ".ncol."\n";

				if(ncol != null)
				{
					// console.log( "find: " + this.findColumnValue(ncol, data) );
		...
		try
		{
			...
			while(queue.length > 0 && t < queue.length)
			{
				...
				if(cancreate && Object.keys(columns).length > 0)
				{
					...
					// console.log( targetName, columns, where );//exit;
